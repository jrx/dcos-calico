...
sudo tee ${CALICO_CNI_CONF_DIR}/${KUBERNETES_CNI_CONF_FILE} <<-'EOF'
   {
     ...
     "plugins": [
       {
         "type": "calico",
         "etcd_endpoints": "https://localhost:62379",
         "etcd_ca_cert_file": "/etc/calico/certs/kubernetes/dcos-ca.crt",
         "etcd_key_file": "/etc/calico/certs/kubernetes/kubernetes-etcd.key",
         "etcd_cert_file": "/etc/calico/certs/kubernetes/kubernetes-etcd.crt",
         "ipam": {
           "type": "calico-ipam"
         },
         "policy": {
